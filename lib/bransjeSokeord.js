...
// Genererer sannsynlige grunnformer av et bøyd ord (bestemt form, flertall) slik
// at f.eks. "pipa", "dørene" og "avløpet" matcher uten at hver bøyningsform må
// listes opp manuelt i ordboken. Bevisst konservativ - kun ord på 5+ tegn
// stemmes, og selve ordboken er domenespesifikk nok til at falske treff i
// praksis krever usannsynlige søk på denne siden (f.eks. "rører" i betydningen
// "stirrer" ville i teorien treffe rørlegger, men er ikke et realistisk søk her).
function bøyningskandidater(ord) {
  const kandidater = new Set([ord]);
  if (ord.length >= 4) {
    if (ord.endsWith('ene')) kandidater.add(ord.slice(0, -3));
    if (ord.endsWith('er')) kandidater.add(ord.slice(0, -2));
    if (ord.endsWith('en')) kandidater.add(ord.slice(0, -2));
    if (ord.endsWith('et')) kandidater.add(ord.slice(0, -2));
    if (ord.endsWith('t')) kandidater.add(ord.slice(0, -1));
    if (ord.endsWith('a')) {
      kandidater.add(ord.slice(0, -1));
      kandidater.add(ord.slice(0, -1) + 'e');
    }
  }
...
// og vanlige bøyningsformer (se bøyningskandidater over), men krever fortsatt
// match på et faktisk nøkkelord eller en avledet grunnform - ingen fri
// skrivefeil-/fuzzy-toleranse.
export function finnNaeringskodeFraTekst(tekst) {
  if (!tekst) return null;
  const ord = tekst.toLowerCase().trim().split(/\s+/);
  for (const w of ord) {
    for (const kandidat of bøyningskandidater(w)) {
      if (BRANSJE_SOKEORD[kandidat]) return BRANSJE_SOKEORD[kandidat];
    }
  }
  return null;
}
