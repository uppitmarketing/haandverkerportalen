...
const BRANSJE_TEKST = {
  ...
  '43.410': 'taklegging og takarbeid',
...
export function genererBedriftFaq(bedrift, naeringsnavn) {
  if (!bedrift) return [];

  const faq = [];
  const bransjeTekst = naeringsnavn ? naeringsnavn.toLowerCase() : 'håndverker';

  faq.push({
    sp: `Er ${bedrift.navn} et aktivt selskap?`,
    sv: bedrift.konkurs
      ? `Nei, ${bedrift.navn} er registrert som konkurs i Brønnøysundregistrene.`
      : bedrift.er_aktiv
        ? `Ja, ${bedrift.navn} er registrert som aktivt i Brønnøysundregistrene.`
        : `${bedrift.navn} er for øyeblikket registrert som inaktivt i Brønnøysundregistrene.`,
  });

  faq.push({
    sp: `Hva er organisasjonsnummeret til ${bedrift.navn}?`,
    sv: `Organisasjonsnummeret til ${bedrift.navn} er ${bedrift.organisasjonsnummer}. Du kan se full oppføring hos Brønnøysundregistrene via lenken på denne siden.`,
  });

  faq.push({
    sp: `Har ${bedrift.navn} nettside?`,
    sv: bedrift.hjemmeside
      ? `Ja, ${bedrift.navn} sin nettside er ${bedrift.hjemmeside.replace(/^https?:\/\//, '').replace(/\/$/, '')}.`
      : `${bedrift.navn} har ikke registrert noen nettside hos Brønnøysundregistrene.`,
  });

  if (bedrift.kommune) {
    faq.push({
      sp: `Hvor holder ${bedrift.navn} til?`,
      sv: `${bedrift.navn} holder til i ${bedrift.poststed ? `${bedrift.poststed}, ` : ''}${bedrift.kommune} kommune, og leverer ${bransjeTekst}-tjenester i området.`,
    });
  }
