...
export const NAERINGSKODER = [
  { kode: '43.210', slug: 'elektriker',   visningsnavn: 'Elektriker' },
  { kode: '43.221', slug: 'rorlegger',    visningsnavn: 'Rørlegger' },
  { kode: '43.222', slug: 'varmepumpe',   visningsnavn: 'Varmepumpemontør' },
  { kode: '43.223', slug: 'ventilasjon',  visningsnavn: 'Ventilasjonsmontør' },
  { kode: '43.320', slug: 'tomrer',       visningsnavn: 'Tømrer' },
  { kode: '41.000', slug: 'byggmester',   visningsnavn: 'Byggmester' },
  { kode: '43.340', slug: 'maler',        visningsnavn: 'Maler' },
  { kode: '43.410', slug: 'taklegger',    visningsnavn: 'Taklegger' },
  { kode: '43.330', slug: 'gulvlegger',   visningsnavn: 'Gulvlegger' },
  { kode: '43.120', slug: 'grunnarbeid',  visningsnavn: 'Grunnarbeid' },
  { kode: '43.910', slug: 'murer',        visningsnavn: 'Murer' },
];
...
// Enkelte næringskoder dekker to reelt ulike fag under ett (f.eks. 43.340
// "Maler- og glassarbeid" - Brønnøysund har ingen finere kode å skille dem
// på). Her kan vi løfte fram bedrifter som navnet tyder tydelig på faktisk
// driver med bransjen søkt, foran nøytrale/tvetydige navn.
const NAVNEHINT_KEYWORDS = {
  maler: ['maler', 'malerfirma', 'malermester'],
};

function harNavnehint(navn, bransjeSlug) {
  const nokkelord = NAVNEHINT_KEYWORDS[bransjeSlug];
  if (!nokkelord || !navn) return false;
  const navnLav = navn.toLowerCase();
  return nokkelord.some(k => navnLav.includes(k));
}

// Standardsortering: navnehint (der vi har nøkkelord for bransjen) først,
// deretter opplyst nettside (lettere å undersøke før man tar kontakt),
// deretter flest ansatte, alfabetisk til slutt. Svake relevans-/
// seriøsitetssignaler - ikke en kvalitetsvurdering eller betalt plassering.
function sorterBedrifter(a, b, bransjeSlug) {
  const aHint = harNavnehint(a.navn, bransjeSlug) ? 0 : 1;
  const bHint = harNavnehint(b.navn, bransjeSlug) ? 0 : 1;
  ...
  const aAnsatte = a.antall_ansatte || 0;
  const bAnsatte = b.antall_ansatte || 0;
  if (aAnsatte !== bAnsatte) return bAnsatte - aAnsatte;
...
export async function getBedrifterByKategoriOgKommune(naeringslug, kommuneslug) {
  ...
  // Sorter samlet liste – navnehint, nettside, ansatte, alfabetisk
  alleBedrifter.sort((a, b) => sorterBedrifter(a, b, naering.slug));
...
export async function sokBedrifter({ navn, naeringskode, kommunenavn, originalTekst }) {
  ...
  if (naeringskode) {
    query1 = query1.eq('naeringskode', naeringskode);
  }
  if (kommunenavn) {
  ...
  // Søk 3: navnet til daglig leder - egen spørring (ikke .or()) for å unngå
  // at kommategn e.l. i søketeksten kan bryte PostgRESTs or()-filtersyntaks.
  let query3 = supabase
    .from('bedrifter')
    .select('*')
    .eq('er_aktiv', true)
    .ilike('daglig_leder_navn', `%${sokTekst}%`)
    .order('har_hjemmeside', { ascending: false })
    .order('navn')
    .limit(20);
  ...
  const alle = [...(res1.data || []), ...(res2.data || []), ...(res3.data || [])];
  ...
  const bransjeSlug = naeringskode ? getNaeringByKode(naeringskode)?.slug : null;
  return Array.from(sett.values()).sort((a, b) => sorterBedrifter(a, b, bransjeSlug));
}
...
// Rangerer kommuner etter antall bedrifter for en gitt bransje - brukt til å
// fremheve de mest folkerike/relevante byene på bransje-indekssiden, slik at
// intern lenkekraft konsentreres mot søk som faktisk har volum (f.eks.
// "rørlegger bergen") i stedet for å spres jevnt over alle ~430 kommuner.
export async function getKommunerRangertForNaering(naeringslug, antall = 16) {
  const naering = getNaeringBySlug(naeringslug);
  if (!naering) return [];

  // Supabase/PostgREST returnerer maks et visst antall rader per kall - uten
  // paginering ville en bransje med mange tusen bedrifter (som elektriker)
  // fått avkuttede tall, og store kommuner som Oslo ville blitt kraftig
  // undertalt siden mesteparten av radene deres faller utenfor det ene
  // svaret. Løkken fortsetter til en helt tom side bekrefter at alt er hentet,
  // uavhengig av hva den faktiske grensen per kall er.
  const antallPerKommune = new Map();
  let fra = 0;
  ...
  while (true) {
    ...
    for (const rad of data) {
      if (rad.kommunenummer) antallPerKommune.set(rad.kommunenummer, (antallPerKommune.get(rad.kommunenummer) || 0) + 1);
    }
    fra += data.length;
  }

  return Array.from(antallPerKommune.entries())
    .map(([nummer, antallBedrifter]) => {
      const kommune = KOMMUNER.find(k => k.nummer === nummer);
      return kommune ? { ...kommune, antallBedrifter } : null;
    })
    .filter(Boolean)
    .sort((a, b) => b.antallBedrifter - a.antallBedrifter)
    .slice(0, antall);
}
